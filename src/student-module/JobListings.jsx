...
const JobListings = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          dispatch(fetchJobs()); // Refresh jobs after applying
        } catch (error) {
  ...
  const handleViewJob = jobId => {
    if (parseFloat(studentDetails?.balance) > 0) {
      Swal.fire({
        icon: 'warning',
        title: 'Pending Payment',
        text: `Please pay the remaining balance of ${studentDetails.balance} to access this job.`,
        confirmButtonText: 'OK',
      });
      return;
    }
    navigate(`/student/job-listings/${jobId}`);
  };
