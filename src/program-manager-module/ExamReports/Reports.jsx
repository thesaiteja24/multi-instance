...
const Reports = () => {
  ...
  const handleDailyClick = batch => {
    setLoadingId(batch.id);
    try {
      navigate(
        `/program-manager/reports/${batch.Batch}/${sessionStorageLocation}`
      );
    } catch (error) {
      console.error('Error navigating to exam mode:', error);
  ...
  return (
    <div className="font-[Inter] w-full max-w-full overflow-x-hidden box-border px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-[10%_80%]">
        <button
          className="text-base sm:text-lg lg:text-xl font-medium text-[var(--color-secondary)]"
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mt-6 mb-7 sm:mt-8">
        ...
        <div className="flex gap-2 items-center mt-4 sm:mt-0">
          ...
          <h2 className="text-base sm:text-lg lg:text-xl font-medium text-[var(--color-secondary)]">
            Batch Wise
          </h2>
        </div>
      </div>
