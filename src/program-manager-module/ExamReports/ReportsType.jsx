...
const ReportsType = () => {
  ...
  const [loading, setLoading] = useState(null); // Track loading card by examType
  ...
  const cardData = [
    {
      ...
      color: '#354128',
      textColor: '#354128',
      route: '/program-manager/reports/daily',
      disabled: false,
      examType: 'Daily-Exam',
    },
    {
      ...
      color: '#4B4B85',
      textColor: '#4B4B85',
      route: '/students-performance-manager/weekly',
      disabled: true,
    ...
    },
    {
      ...
      color: '#FF6000',
      textColor: '#FF6000',
      route: '/students-performance-manager/monthly',
      disabled: true,
    ...
    },
    {
      ...
      color: '#21B1CA',
      textColor: '#21B1CA',
      route: '/students-performance-manager/grand',
      disabled: true,
      examType: 'Grand-Exam',
    },
  ];
  ...
  const handleCardClick = async card => {
    if (card.disabled || loading) return;

    setLoading(card.examType); // Start loading for this card
    try {
      // Simulate a brief loading delay for visual feedback (optional)
      await new Promise(resolve => setTimeout(resolve, 500));

      // Navigate to card.route with batch, location, and examType
      navigate(card.route, {
        state: {
          batch,
          location,
      ...
      toast.error('Error navigating to exam report', {
        autoClose: false,
        closeOnClick: true,
        closeButton: true,
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center gap-10 h-[calc(100%-44px)] px-6 py-4 sm:px-6 sm:py-6">
                ...
                <button
                  ...
                  style={{
                    backgroundColor:
                      card.disabled || loading === card.examType
                        ? '#A0A0A0'
                        : card.color,
                  }}
                  disabled={card.disabled || loading === card.examType}
                >
                  {loading === card.examType ? (
                    <div
