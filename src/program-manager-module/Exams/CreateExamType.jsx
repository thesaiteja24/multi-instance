...
const CreateExamType = () => {
  ...
  const cardData = [
    ...
    },
    {
      ...
      disabled: true, // Disabled Weekly Exam
    ...
    },
    {
      ...
      disabled: true, // Disabled Monthly Exam
    ...
    },
    {
      ...
      disabled: true, // Disabled Grand Test
      examType: 'Grand-Exam',
    },
  ];
  ...
  const handleCardClick = async (card, index) => {
    ...
    try {
      ...
      // Navigate to /set-exam with examData and batch
      navigate('/program-manager/schedule-exam/create', {
        state: {
          examData: data.data,
          batch: { Batch: batch, location: location },
        },
      });
    } catch (error) {
      const err = error.response?.data;
      if (error.response?.status === 409) {
        toast.warning(err?.message || 'Conflict in exam status', {
          autoClose: 5000,
          closeOnClick: true,
          closeButton: true,
          style: { width: '500px', whiteSpace: 'pre-wrap' },
        ...
        const errorMessage =
          err?.error || err?.message || 'Error processing exam data';
        const tags = err?.tags ? `\nTags: ${err.tags.join(', ')}` : '';
        toast.info(`${errorMessage}${tags}`, {
          autoClose: false,
          closeOnClick: true,
          closeButton: true,
          style: { width: '500px', whiteSpace: 'pre-wrap' },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center font-[Inter]">
          {cardData.map((card, index) => (
            <div
              ...
              style={{ height: '260px', width: '100%', maxWidth: '342px' }}
            >
              <div
                className="w-full h-[44px] rounded-t-[28.25px]"
              ...
              <div className="flex flex-col items-center justify-center gap-10 h-[calc(100%-44px)] px-6 py-4 sm:px-6 sm:py-6">
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor:
                      loadingCardIndex === index ? 'not-allowed' : 'pointer',
                  }}
                  disabled={card.disabled || loadingCardIndex === index}
                >
