...
const ProgramManagersManagement = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/manager`
      );
  ...
  const handleOpenModal = (manager = null) => {
    if (manager) {
      const match = manager.PhNumber?.match(/^(\+\d+)(\d{10})$/) || [
        '',
        '+91',
        '',
      ];
  ...
  const handleSave = async () => {
    ...
    if (!phoneRegex.test(phoneNumber) || /^(\d)\1{9}$/.test(phoneNumber)) {
      Swal.fire({
        icon: 'error',
    ...
    const isDuplicatePhone = data.some(
      manager =>
        manager.PhNumber === selectedCountryCode?.value + phoneNumber &&
            ...
            {Array.isArray(data) && data.length > 0 ? (
              data.map((item, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-3">{item.userType || 'manager'}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-[13px] w-[500px] p-[30px] flex flex-col gap-5 relative">
            <h3 className="text-[#00007F] text-center text-[24px] font-semibold leading-[36px]">
              {formData.id ? 'Edit Manager' : 'Add Manager'}
            </h3>
...
export default ProgramManagersManagement;
