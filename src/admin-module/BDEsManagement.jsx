...
const BDEsManagement = () => {
  ...
  const handleOpenModal = (bde = null) => {
    if (bde) {
      const match = bde.PhNumber?.match(/^(\+\d+)(\d{10})$/) || ['', '+91', ''];
  ...
  const handleCloseModal = () => {
    ...
    setIsSaving(false);
    setFormData({ id: '', name: '', email: '', location: '', userType: 'BDE' });
    setSelectedCountryCode(
      countryCodes.find(c => c.value === '+91') || {
        value: '+91',
        label: '+91',
  ...
  const handleSave = async () => {
    ...
    if (!emailRegex.test(formData.email)) {
      Swal.fire({
        icon: 'error',
        title: 'Invalid Email',
        text: 'Email must be from codegnan.com, destinationtechs.com, or gmail.com.',
      });
      return;
    }
    ...
    if (!phoneRegex.test(phoneNumber)) {
      Swal.fire({
        ...
        text: 'Phone number must be 10 digits starting with 6, 7, 8, or 9.',
      });
      return;
    }
    ...
    const formattedData = {
      ...formData,
      PhNumber: selectedCountryCode?.value + phoneNumber,
    };
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        Swal.fire({
          icon: 'success',
          title: formData.id
            ? 'BDE updated successfully!'
            : 'BDE added successfully!',
        });
        fetchData();
        handleCloseModal();
      }
        ...
        <table className="min-w-[1200px] w-full table-auto">
          <thead className="bg-[#00007F] text-white text-left text-[18px] font-semibold leading-[28px]">
            <tr>
              ...
              <th className="px-6 py-3 border-b border-[#E9EAEB] w-[300px]">
                Email
              </th>
              <th className="px-6 py-3 border-b border-[#E9EAEB] w-[200px]">
                Phone
          ...
          <div className="bg-white rounded-[13px] w-[500px] p-[30px] flex flex-col gap-5 relative">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="border border-[#00007F] rounded px-4 py-3 text-[#666666]"
                value={formData.name}
                onChange={e =>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center border border-[#00007F] rounded">
                ...
                <input
                  ...
                  className="flex-1 px-4 py-3 text-[#666666] border-none focus:outline-none"
                  value={phoneNumber}
