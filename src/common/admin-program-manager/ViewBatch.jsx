...
const ViewBatch = () => {
  ...
  useEffect(() => {
    ...
    } else if (![COLLEGE_CODE].includes(sessionStorageLocation)) {
  ...
  // Filter batches and update status
  useEffect(() => {
    let filtered =
      locationFilter === 'all'
        ? batchesList
  ...
  }, [batchesList, locationFilter]);

  // Reset page when filtered batches change
  useEffect(() => {
    setCurrentPage(1);
  }, [filteredBatches]);
  ...
  const handleSave = async batchId => {
    if (editedData.Duration === 'Invalid Dates') {
  ...
  const handleViewMore = batch => {
    if (userType === ADMIN || userType === SUPER_ADMIN) {
      navigate('/admin/view-batch/schedule', { state: { batch } });
    } else {
      navigate('/program-manager/view-batch/schedule', { state: { batch } });
    }
  };
