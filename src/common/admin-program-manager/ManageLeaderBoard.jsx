...
const ManageLeaderBoard = () => {
  ...
  useEffect(() => {
    if (!batchesList.length && !batchesListLoading && !batchesListError) {
      dispatch(getBatches(location));
    }
  }, [dispatch, location, batchesList, batchesListLoading, batchesListError]);
  ...
  useEffect(() => {
    if (activeTab === 'Overall') {
      fetchData('Overall');
    } else if (activeTab === 'Class' && !batchesListLoading && selectedBatch) {
      fetchData('Class', selectedBatch);
    }
  }, [activeTab, selectedBatch, batchesListLoading, fetchData]);
  ...
  useEffect(() => {
    if (
      ...
      setSelectedBatch(batchesList[0]?.Batch || '');
    } else if (activeTab === 'Overall') {
      setSelectedBatch('');
    }
  }, [activeTab, batchesList, batchesListLoading, selectedBatch]);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-red-500 text-lg font-semibold">
          ...
          <button
            onClick={() => dispatch(getBatches(location))}
      ...
      >
        {['Class', 'Overall'].map(tab => (
          <button
            ...
            } ${isLoading || batchesListLoading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            {tab}
          </button>
        ))}
        ...
        <div className="w-full max-w-md mb-6">
          <div className="relative w-full h-12 rounded-full bg-blue-800 p-1">
            <select
              ...
              disabled={batchesListLoading || isLoading}
