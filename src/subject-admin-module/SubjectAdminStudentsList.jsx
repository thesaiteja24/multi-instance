...
export default function SubjectAdminStudentsList() {
  ...
  useEffect(() => {
    ...
    if (location) {
      try {
        if (location === 'all') {
          setIsAdmin(true);
          setSearchLocation(COLLEGE_CODE);
        } else if ([COLLEGE_CODE].includes(location)) {
  ...
  useEffect(() => {
    if (!error) {
      ...
      const effectiveLocation =
        searchLocation !== 'SelectLocation' ? searchLocation : COLLEGE_CODE;
      if (batchesList.length === 0) {
        dispatch(getBatches(isAdmin ? effectiveLocation : searchLocation));
      }
    }
  }, [dispatch, searchLocation, isAdmin, error, batchesList.length]);
  ...
  const filteredBatches = userType
    ? batchesList.filter(batch => {
        ...
        if (!batch.Batch.startsWith(prefix)) {
          console.warn(
            `Invalid batch prefix for ${userType}: ${batch.Batch} (ID: ${batch._id})`
          );
          return false;
        }
        return true;
  ...
  const exportToExcel = () => {
    ...
    saveAs(
      blob,
      `students-list_${userType || 'unknown'}_${searchLocation}.xlsx`
    );
  };
  ...
  const filteredStudents = (studentsList || []).filter(student => {
    ...
    const batchPrefix =
      userType === 'Python' ? 'PFS-' : userType === 'Java' ? 'JFS-' : '';
    if (userType && batchNo && !batchNo.startsWith(batchPrefix)) {
      console.warn(
        `Invalid BatchNo prefix for ${userType}: ${batchNo} (Student ID: ${studentId})`
      );
      return false;
    }
    ...
    return (
      ...
      (effectiveLocation === '' ||
        studentLocation.toLowerCase().includes(effectiveLocation.toLowerCase()))
    );
  });
          ...
          >
            ...
            <div className="relative w-full">
              <select
                ...
                disabled={batchesListLoading || filteredBatches.length === 0}
            ...
            <div className="flex items-center justify-center gap-4 bg-[#00007F] text-white p-3 rounded-md text-sm sm:text-base hover:bg-blue-800 transition">
              ...
              <button onClick={exportToExcel} disabled={totalStudents === 0}>
                Download Excel
              </button>
            </div>
          </div>
