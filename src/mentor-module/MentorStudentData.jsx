...
export default function StudentsList() {
  ...
  const uniqueBatches = scheduleData
    ? Array.from(
        new Set(scheduleData.flatMap(schedule => schedule.batchNo || []))
      ).map(batch => ({ Batch: batch, _id: batch }))
    : [];
  ...
  const filteredStudents = uniqueStudentsList.filter(student => {
    ...
    const studentLocation = student?.collegeName || '';
  ...
  return (
    <div className="p-5 font-[Inter]">
      <div>
        <h2 className="font-semibold text-[var(--color-secondary)] text-[24px] text-center">
          Students List (
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4 bg-[var(--color-secondary)] text-white p-3 rounded-md text-sm sm:text-base hover:bg-blue-800 transition">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-[var(--color-secondary)] text-white font-semibold text-[16px]">
                ...
                <th className="px-6 py-3 text-left">BatchNO</th>
            ...
            <tbody>
              {currentStudents.map((student, index) => (
                <tr
                  key={student.id}
                  className={index % 2 === 0 ? 'bg-[#FFFFFF]' : 'bg-[#EFF0F7]'}
                >
                  ...
                  <td className="px-6 py-5">{student.collegeName || '__'}</td>
                  <td className="px-6 py-5">{student.qualification || '__'}</td>
                  <td className="px-6 py-5">{student.department || '__'}</td>
                </tr>
              ))}
            </tbody>
          </table>
