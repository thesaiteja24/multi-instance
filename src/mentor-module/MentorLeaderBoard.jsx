...
const MentorLeaderBoard = () => {
  ...
  const { scheduleData, mentorStudentsListLoading, mentorStudentsListError } =
  ...
  useEffect(() => {
    if (!scheduleData.length) {
      dispatch(getMentorStudentsThunk());
    }
  ...
  if (mentorStudentsListError) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-red-500 text-lg font-semibold">
          Error: {mentorStudentsListError}
          <button
            onClick={() => dispatch(getMentorStudentsThunk())}
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-red-500 text-lg font-semibold">
          Error: {error}
          <button
            onClick={() => fetchData(activeTab, selectedBatch)}
            className="ml-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
          >
            Retry
          </button>
    ...
    <div className="flex flex-col items-center justify-start min-h-screen font-sans text-white mt-16 px-4">
      ...
      >
        {['Class', 'Overall'].map(tab => (
          <button
            ...
            disabled={isLoading || mentorStudentsListLoading}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 h-10 rounded-full text-sm font-medium transition-all duration-300 ${
              activeTab === tab
                ? 'bg-white text-blue-900'
                : 'text-white hover:bg-blue-700'
            } ${isLoading || mentorStudentsListLoading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            {tab}
          </button>
        ))}
      ...
      {activeTab === 'Class' && (
        <div className="w-full max-w-md mb-6">
          {batches.length === 0 ? (
            <p className="text-center text-lg font-semibold text-blue-800 bg-white rounded-full px-4 py-2">
              No batches available
            </p>
          ) : batches.length === 1 ? (
            <p className="text-center text-base font-medium text-blue-900 bg-white rounded-full px-4 py-2">
              Batch: {batches[0]}
            </p>
          ) : (
            <div className="relative w-full h-12 rounded-full bg-blue-800 p-1">
              <select
                value={selectedBatch}
                onChange={e => {
                ...
                disabled={isLoading || mentorStudentsListLoading}
                className="w-full h-full px-4 py-2 bg-white text-blue-900 text-sm font-medium rounded-full outline-none appearance-none cursor-pointer"
              >
              ...
              <svg
                className="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-900 pointer-events-none"
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="currentColor"
